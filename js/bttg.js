...
calcButton.onclick = () => {
    ...
    resultBox.innerHTML = `<div>Trả trước: <span class="percent">${percent}%</span></div><div>Phần còn lại trả góp: <span class="amount">${formatNum(remain)} VNĐ</span></div>`;
...
addInsurance.onclick = () => {
    ...
    if (!rate) {
        insuranceBox.innerHTML = `<div>Phần còn lại trả góp: <span class="amount">${formatNum(originalLoanAmount)} VNĐ</span></div>`;
        ...
        insuranceBox.innerHTML = `
            <div>Phần còn lại trả góp: <span class="amount">${formatNum(originalLoanAmount)} VNĐ</span></div>
            <div>Thêm bảo hiểm: <span class="percent">${rate}%</span></div>
            <div>Phí bảo hiểm: <span class="amount1">+${formatNum(fee)} VNĐ</span></div>
            <div>Đã bao gồm bảo hiểm: <span class="amount">${formatNum(total)} VNĐ</span></div>
        `;
        insuranceBox.style.display = 'block';
...
function captureAndDownload(mode) {
    ...
    const info = `
        ...
        ${insuranceText}
        <p><strong>Phần còn lại trả góp:</strong> ${safeValue(loanInput)} VNĐ</p>
